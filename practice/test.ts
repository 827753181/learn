...
/* class myPromise {
  ...
  _handle({ onFulfilled, onReject, resolve, reject }) {
    if (this.state === "pending") {
      ...
      //否则说明是其他情况进来的，直接内置promise的执行函数
      const runFn = this.state === "fulfilled" ? onFulfilled : onReject;
      const inputValue = this.state === "fulfilled" ? this.value : this.reason;
      if (!runFn) {
        //如果没有传入需要run的function
        (this.state === "fulfilled" ? resolve : reject)(inputValue);
        return;
      }
      try {
        const value = runFn(inputValue);
  ...
  _resolve(value) {
    ...
    if (value && value.then === "function") {
      const valueThen = value.then;
      valueThen.call(this, this._resolve.bind(this), this._reject.bind(this));
      return;
    }
